...
if [ -n "$JAVA_HOME" ] ; then
    if [ -x "$JAVA_HOME/bin/java" ] ; then
        # Now GTC-553: Java 1.7.0_72 and higher on Windows 7+ will not set the executable bit
        # for java.exe, so we must check for that.
...
if $cygwin ; then
    ...
    ROOTDIRSRAW=`find / -maxdepth 1 -typed -printf '%p|' 2>/dev/null`
    SEP=""
    for d in /dev /bin /lib /usr /tmp ; do
      if [ -d "$d" ] ; then
        ROOTDIRSRAW="${ROOTDIRSRAW}${SEP}${d}"
        SEP="|"
      fi
    done
...
# by default we should be in the correct project dir, but the script itself can be elsewhere
# so we should change back to the project dir
exec "$JAVACMD" "$@"
